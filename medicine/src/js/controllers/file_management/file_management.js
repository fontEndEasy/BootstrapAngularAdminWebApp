...
app.controller('fileManagementCtrl', function($rootScope, $scope, $state, $stateParams, $http, toaster, $modal, editableThemes, editableOptions) {
    ...
    function saveFileInfo(name, mimeType, size, key, hash) {
        return $http({
            url: app.url.drugEnterpriseEndUpload,
            method: 'post',
            // headers: {
            ...
            // },
            data: {
                access_token: localStorage.getItem('yy_access_token'),
                fileName: name,
                bizType: mimeType,
                mimeType: mimeType,
                size: size,
    ...
    $scope.fileFormatIdentify = function(item) {
        ...
        if (
            ...
            return item.url || fileTypeUrl + 'image.png';
    ...
    function getMyFile(pageIndex, pageSize, mode, type, sortAttr, sortType, fileNameKey) {
        ...
        $http({
            url: app.url.drugEnterpriseQueryFile,
            method: 'post',
            data: {
                access_token: localStorage.getItem('yy_access_token'),
                ...
                fileName: fileNameKey || $scope.fileNameKey || null
            }
        }).then(function(response) {
            var rep = response.data;
            if (rep && rep.resultCode == 1) {
    ...
    function deleteFile(ids, mode) {
        ...
        $http({
            url: app.url.drugEnterpriseDeleteFileById,
            method: 'post',
            data: {
                access_token: localStorage.getItem('yy_access_token'),
                fileId: ids[0]
