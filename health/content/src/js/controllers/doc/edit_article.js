app.controller('EditCtrl', function($scope, $timeout, utils, $http, $modal, Upload, toaster, $state, $stateParams) {
    ...
    $scope.saveDoc = function() {
        ...
        if (isEdit == true) {
            ...
            success(function(data, status, headers, config) {
                if (data.resultCode == 1) {
                    if ($state.includes('app.doc.edit_article')) {
                        toaster.pop('success', '', '保存成功,3秒钟后返回文章列表');
                        setTimeout(function() {
                            $state.go('app.doc.doctor_doc', {}, {
                                reload: true
                            });
                        }, 3000);
                    } else if ($state.includes('doc.edit_article')) {
                        window.opener.reflashData();
                        toaster.pop('success', '', '保存成功,3秒钟后返回');
                        setTimeout(function() {
                            $state.go('doc.article', {
                                id: $stateParams.id,
