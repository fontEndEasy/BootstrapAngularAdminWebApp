...
app.controller('CustomerService', ['$scope', '$http', '$state', '$rootScope', 'utils', 'uiLoad', 'JQ_CONFIG', 'modal',
    function ($scope, $http, $state, $rootScope, utils, uiLoad, JQ_CONFIG, modal) {
        $scope.isPass = true;
        ...
        $scope.submit = function () {
            ...
            if((!formParam.hospitalId && formParam.hospitalId != '0') || formParam.hospitalId == ' '){
                modal.toast.warn('医院id不存在，您需要手动选择（或新建）此医院！');
                return;
            }
